    ...
    <div>
      ...
      <div class="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface)] p-6 sm:p-8 mb-10 shadow-sm overflow-hidden">
        ...
        <div v-if="completedTasksWithDetails.length" class="space-y-4">
          <NuxtLink
            v-for="task in completedTasksWithDetails"
            :key="task.slug"
            :to="`/dsa/problems/${task.slug}`"
      ...
      <div class="flex flex-wrap gap-4">
        <NuxtLink to="/dsa" class="hero-btn hero-btn--primary !rounded-2xl shadow-lg shadow-emerald-500/20">
          <Library class="w-5 h-5" />
          Browse Problems
        </NuxtLink>
        <NuxtLink to="/dsa/assessment" class="hero-btn hero-btn--secondary !rounded-2xl">
          <Timer class="w-5 h-5" />
