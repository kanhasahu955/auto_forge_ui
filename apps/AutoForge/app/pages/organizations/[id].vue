  ...
  <div class="dash-page px-4 sm:px-6 py-8">
    ...
    <template v-else-if="org">
      ...
      <div class="flex items-center gap-4 mb-8">
        <NuxtLink to="/organizations" class="text-sm text-[var(--app-text-muted)] hover:text-[var(--app-accent)] transition-colors shrink-0">
          ← Back
        </NuxtLink>
        <div class="flex items-center gap-4 flex-1 min-w-0">
          <AppAvatar :size="52" class="!bg-[var(--app-accent)]/12 !text-[var(--app-accent)] !font-semibold shrink-0">
            {{ org.name?.charAt(0)?.toUpperCase() ?? 'O' }}
          </AppAvatar>
          <div class="min-w-0">
            <h1 class="text-lg font-semibold text-[var(--app-text)] truncate">{{ org.name }}</h1>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        <AppCard v-for="stat in stats" :key="stat.label" class="dash-stat-card">
      ...
      <div class="grid gap-5 lg:grid-cols-2 mb-8">
        <AppCard class="dash-section-card">
          ...
          <ul v-else class="space-y-1">
            <li v-for="a in activities" :key="a.id" class="dash-list-item flex justify-between items-center">
              <span class="font-medium text-[var(--app-text)] text-sm truncate">{{ a.name }}</span>
        ...
        <AppCard class="dash-section-card">
          <template #header>Assessments</template>
          ...
          <ul v-else class="space-y-1">
            <li v-for="a in assessments" :key="a.id" class="dash-list-item flex justify-between items-center">
              <span class="font-medium text-[var(--app-text)] text-sm truncate">{{ a.name }}</span>
      ...
      <AppCard class="dash-section-card">
        ...
        <div v-else class="space-y-5">
          <div v-if="members.length">
            ...
            <div class="space-y-1">
              <div v-for="m in members" :key="m.id" class="dash-list-item flex items-center gap-3">
          ...
          <div v-if="invitations.length">
            ...
            <div class="space-y-1">
              <div v-for="inv in invitations" :key="inv.id" class="dash-list-item flex items-center gap-3 bg-amber-500/5 border border-amber-500/20 rounded-lg">
...
const stats = computed(() => [
  ...
  { label: 'Pending', value: invitations.value.length },
])
